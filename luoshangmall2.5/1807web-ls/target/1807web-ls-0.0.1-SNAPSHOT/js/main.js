/**
 * main.js http://www.codrops.com
 * 
 * Licensed under the MIT license.
 * http://www.opensource.org/licenses/mit-license.php
 * 
 * Copyright 2015, Codrops http://www.codrops.com
 */
;(function(window) {
	...
	var bodyEl = document.body, 
		...
		// scale body when zooming into the items, if not Firefox (the
		// performance in Firefox is not very good)
...
// closeContentCtrl.addEventListener('click', closeContent);
	...
	function openItem(item) {
		...
		if( bodyScale ) {
			dynamics.animate(bodyEl, { scale: bodyScale }, { type: dynamics.easeInOut, duration: 1000 });
		}
		...
		// onEndTransition(zoomer, function() {
			...
			// reset zoomer transform - back to its original position/transform
	...
	function closeContent() {
		...
		onEndTransition(contentItem, function(ev) {
			...
			// scale up - behind the scenes - the item again (without
			// transition)
			applyTransforms(zoomer);
