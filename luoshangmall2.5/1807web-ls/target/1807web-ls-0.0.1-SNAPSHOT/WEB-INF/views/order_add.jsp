	...
	<div class="warp">
		<form name="form1" method="post" action="${app}/order/addOrder">
			...
			<div id="forminfo">
				...
				<textarea name="receiverAddress" id="textarea" cols="45" rows="5"></textarea>
		...
		<c:forEach items="${cartList}" var="cartItem" varStatus="status">
				<tr>
					...
					<td><img src="http://image.luoshangmall.cn/${cartItem.productImage}" width="90" height="90" /></td>
					...
					<td>${cartItem.size}</td>
					<input type="hidden" name="orderItems[${status.index}].size" value="${cartItem.size}"/>
			...
			<div class="add_orderbox">
				<input name="" type="submit" value="确认订单" class="add_order_but">
			</div>
		</form>
	</div>
</body>
