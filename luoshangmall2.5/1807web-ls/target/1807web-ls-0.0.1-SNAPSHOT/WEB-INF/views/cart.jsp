  ...
  <head>
	...
	<script type="text/javascript">
		$(function(){
			$(".del").click(function(){
				if(confirm("您确定删除吗？")){
					var size=$(this).parent().prev().prev().prev().text();
					var id = $(this).parent().parent().find("input[class=buyNumInp1]").attr("id");
					window.location.href="${app}/cart/deleteCart/"+id+"/"+size;
				}
			});
			$(".delNum").click(function(){
				...
				//获取商品的码数
				var size=$(this).parent().prev().text();
				//2获取输入框中数量
				...
				if(num>0){
					//4、跳转
					window.location.href="${app}/cart/editCart/"+id+"/"+num+"/"+size;
				}else{
					if(confirm("您确认删除吗？")){
						window.location.href="${app}/cart/deleteCart/"+id+"/"+size;
					}
...
<!--尺码-->	<span id="title_size">尺码</span>
	...
	<c:forEach items="${cartList}" var="cartItem">
		<div id="prods">
			...
			<img src="http://image.luoshangmall.cn/${cartItem.productImage}" width="90" height="90" />
			<span id="prods_name" >${cartItem.productName}</span>
			<span id="prods_price">${cartItem.productPrice}</span>
			<span id="prods_size">${cartItem.size}</span>
		...
		<div id="total">
			...
			<div id="total_2">	
				<a id="goto_order" href="${app }/order/cart-order">添加到订单</a>
			</div>
		</div>
	</div>
</body>
