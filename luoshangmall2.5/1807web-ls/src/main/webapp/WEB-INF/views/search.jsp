			...
			<div id="spulist-pager">
				 ...
				 <%--  	<c:if test="${requestScope.page.totalPage>1}"> --%>
						  ...
						  <c:if test="${page.currentPage eq 1}">
						    <a href="${app}/query?q=${page.info }&currentPage=2">下一页</a>
						  	<a href="${app}/query?q=${page.info }&currentPage=${page.totalPage }">尾页</a>
						  </c:if>
						  <!-- 非第一页也非最后一页-->
						  <c:if test="${page.currentPage>1 and page.currentPage<page.totalPage}">
						  	<a href="${app}/query?q=${page.info}&currentPage=1">首页</a>
						  	<a href="${app}/query?q=${page.info}&currentPage=${page.currentPage-1}">上一页</a>
						  	<a href="${app}/query?q=${page.info}&currentPage=${page.currentPage+1 }">下一页</a>
						  	<a href="${app}/query?q=${page.info}&currentPage=${page.totalPage}">尾页</a>
						  </c:if>
						  <!-- 最后一页情形 -->
